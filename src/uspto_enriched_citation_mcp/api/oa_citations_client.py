...
# OA Citations gets its own circuit breaker instance (bulkhead isolation from
# Enriched Citations — E6): a failure streak against one USPTO API must not
# open the circuit for the other. BaseCitationClient.__init__ resolves the
# same named instance via _CACHE_KEY_PREFIX ("oa"); this module-level name is
# kept for tests/tools that reset breaker state directly.
uspto_api_breaker = get_circuit_breaker(
    "oa", failure_threshold=3, recovery_timeout=30.0, success_threshold=2
)
