...
Operator-relevant summary of the July 2026 audit-remediation series. See `CLAUDE.md` for full technical detail and file references.

- **Admin-tool registration gating** — `citations_manage_users` is registered only when `CITATIONS_ENABLE_USER_MANAGEMENT=true` (default off); absent from `tools/list` entirely otherwise.
- **Fail-fast scope verification** — startup refuses to run if the admin scope gate fails to attach to the registered admin tool during OAuth mode.
- **Query validation (Lucene-safe)** — validates Lucene syntax via field whitelists (VALID_FIELDS for Enriched v3, OA_VALID_FIELDS for Office Action v2), prevents injection patterns (script/template/command), caps query length and nesting depth, validates balanced parentheses/brackets to prevent DoS.
- **Security-log content minimization** — masked emails/IPs, no raw tokens, query fingerprints only (SHA256 hashes); SanitizingFilter on all handlers catches URLs and paths before reaching any output stream.
- **Refresh-token family revocation** — replay of a spent/rotated OAuth refresh token revokes every live token for that identity.
- **Shared cross-process USPTO rate limiter** — see the README section below; cross-process token bucket + bounded concurrency-slot pool arbitrated via POSIX file locks (crash-safe by kernel design).
- **OA field validation** — OA Citations v2 tools use their own field whitelist (OA_VALID_FIELDS) separate from Enriched v3 (VALID_FIELDS), fixing live regression where OA tools' validator was dead code until audit fixes made it real (2026-07-09).
...
### 🌐 **Rate Limiting Is Process-Local By Design (M4, accepted)**

Every rate limiter in this codebase — the outbound `util/rate_limiter.RateLimiter`
token buckets and the OAuth-surface `_FixedWindowRateLimiter` in `auth/provider.py`
— keeps its state in an in-process dict/lock. This is a deliberate scope
decision, not an oversight: the service deploys as a **single container**
behind a gateway/reverse proxy, so
one process's counters are the whole deployment's counters. If this MCP is
ever horizontally scaled (multiple replicas behind a load balancer), revisit
this: either move to a shared-state limiter (Redis + Lua, or similar) or push
rate limiting to the reverse-proxy/API-gateway layer in front of all
replicas, since per-replica in-memory limits would each enforce their own
quota independently and the effective rate would scale with replica count.
