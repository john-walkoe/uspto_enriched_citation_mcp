...
class TestCaching:
    ...
    def test_fields_cache(self, monkeypatch):
        """Test 5.2: Fields cache works correctly."""
        from uspto_enriched_citation_mcp.util import cache as cache_module
        from uspto_enriched_citation_mcp.util.cache import get_fields_cache

        # Lazy singleton: reset so this test gets a fresh, empty instance
        # regardless of what ran before it.
        monkeypatch.setattr(cache_module, "_fields_cache", None)
        cache = get_fields_cache(ttl_seconds=60, max_size=10)

        # Cache should start empty (public API: get_stats(), not private _cache)
    ...
    def test_search_cache(self, monkeypatch):
        """Test 5.3: Search cache works correctly."""
        from uspto_enriched_citation_mcp.util import cache as cache_module
        from uspto_enriched_citation_mcp.util.cache import get_search_cache

        # Lazy singleton: an earlier test creates the cache with the default
        # max_size=100, so max_size=5 here would be silently ignored (the
        # order-dependent failure from the testing audit). Reset it.
        monkeypatch.setattr(cache_module, "_search_cache", None)
        ...
        # Cache should still have 5 items (max_size) — public API: get_stats(),
        # not private _cache
