...
```
**Expect:** ~14M numFound (raw OA citations, ~3x the enriched dataset). Records have referenceIdentifier, actionTypeCategory, legalSectionCode present where populated.
...
| OA dataset size | ~12M records vs ~4.2M enriched (for TC:2100); broader raw coverage |
...
| OA cross-check gap | App 13487597: 0 enriched citations vs 19 OA citations — OA v2 has broader raw coverage of records not yet AI-processed into enriched set |
