...
class TestValidateConfigPath:
    """Tests for the path-traversal guard, FieldManager._validate_config_path
    (Phase 6A audit item 7 — this is a security boundary; lock its behavior).
    """

    def test_accepts_legit_config_path(self):
        """The real, in-project config path is accepted and resolved."""
        config_path = Path(__file__).parent.parent / "field_configs.yaml"
        manager = FieldManager(config_path)
    ...
    def test_rejects_symlink_escaping_project(self, tmp_path):
        ...
        it at a location outside both roots is rejected (not naively allowed
        because the symlink's own path looked fine)."""
        target_outside = tmp_path / "outside_target.yaml"
        target_outside.write_text("predefined_sets: {}")
